...
*/
  ...
  hy = 3.0*m;
  hz = 1.0*m;
  G4VSolid* shieldS = new G4Box("shieldS", hx, hy, hz);
  G4LogicalVolume* shieldLV
    = new G4LogicalVolume(shieldS, concrete, "Shield");

  // Rotation
  G4RotationMatrix* shieldRotation = new G4RotationMatrix();
  *shieldRotation = *secondArmRotation;

  // Position
  zpos = 5.*m + 3.*m + 1.5*m;
  G4ThreeVector shieldPosition(0, 0, zpos);
  shieldPosition.rotateY(-armAngle);
  ...
  // Subdivide shield in slices:
  G4int nofSlices = 10;
  G4double sliceHz = hz / nofSlices;
  G4VSolid* shieldSliceS = new G4Box("shieldSliceS", hx, hy, sliceHz);
  G4LogicalVolume* shieldSliceLV
    = new G4LogicalVolume(shieldSliceS, concrete, "ShieldSlice");
  G4double sliceThickness = 2*sliceHz;
  new G4PVReplica("ShieldSlice",       // its name
		    shieldSliceLV,     // its logical volume
		    shieldLV,          // its mother
		    kZAxis,            // axis of replication
		    nofSlices,         // number of replica
		    sliceThickness);   // width of replica
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  // Attach biasing operator to slices of calorimeter:
  // Use BTW a facility to retrieve a logical volume pointer by name:
  G4LogicalVolume* shieldSlice = G4LogicalVolumeStore::GetInstance()->GetVolume("ShieldSlice");
  BiasingOperator* biasingOperator = new BiasingOperator();
  biasingOperator->AttachTo(shieldSlice);
}
