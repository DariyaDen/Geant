...
ScreenSD::ScreenSD(const G4String& name, G4int ntupleId)
  ...
  if ( !fIdxInitialized )
    {
      ...
      fPhotoGamma = G4PhysicsModelCatalog::GetModelIndex("phot_fluo");   // fluorescence from photoelectric
      fComptGamma = G4PhysicsModelCatalog::GetModelIndex("compt_fluo");  // fluorescence from compton
      fPhotoAuger = G4PhysicsModelCatalog::GetModelIndex("phot_auger");  // auger e- from photoelectric
      fComptAuger = G4PhysicsModelCatalog::GetModelIndex("compt_auger"); // auger e- from compton
      fPixeGamma  = G4PhysicsModelCatalog::GetModelIndex("gammaPIXE");   // gamma from PIXE
      fPixeAuger  = G4PhysicsModelCatalog::GetModelIndex("e-PIXE");      // e- from PIXE
      ...
      for ( G4int iEntry = 0 ; iEntry < G4PhysicsModelCatalog::Entries() ; iEntry++ ) {
        G4cout << " idx = "
               << iEntry << " "
               <<  G4PhysicsModelCatalog::GetModelNameFromIndex(iEntry) << G4endl;
      }
