                                    ...
                                    G4TouchableHistory* /*history*/)
{
  ...
  EmCalorimeterHit* hit = (*fHitsCollection)[layerNumber];
  if ( ! hit ) {
  ...
  if ( step->GetTrack()->GetDefinition()->GetPDGCharge() != 0.0 ) {
    EmCalorimeterHit* hitCharged = (*fHitsCollection)[10];
    // Add values
    hitCharged->AddEdep(edep);
  }
  else {
    EmCalorimeterHit* hitNeutral = (*fHitsCollection)[11];
    // Add values
  ...
  if ( step->GetTrack()->GetParentID() == 0 ) {
    EmCalorimeterHit* hitPrimary = (*fHitsCollection)[12];
    // Add values
    hitPrimary->AddEdep(edep);
  }
  else {
    EmCalorimeterHit* hitOthers = (*fHitsCollection)[13];
    // Add values
